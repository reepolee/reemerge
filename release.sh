...
#
# reemerge is pure Rust (no C/native deps), so every target cross-compiles cleanly
# from macOS:
...
if [ "$new_commits" -gt 0 ] && [ "$force_skip_bump" = false ] && [ "$migration_mode" = false ]; then
	...
	echo "═══ reemerge release $new_version (all targets) ═══"
	...
	echo "═══ reemerge release $version (all targets) ═══"
